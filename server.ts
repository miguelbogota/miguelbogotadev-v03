...
export default {
  fetch: async (req: Request) => {
    ...
    if (projectId) {
      ...
      if (!project) return new Response(NotFoundRoute({ theme, content }), responseConfig);
      return new Response(ProjectRoute({ theme, content, project }), responseConfig);
    }

    return new Response(HomeRoute({ theme, content, projects }), responseConfig);
  },
};
